...
/// Everything that interprets a `TrackedParcel` in terms of `TsuiseKit`. Kept in
/// a separate, app-only file so the bare `@Model` in `TrackedParcel.swift` can be
/// compiled into the share extension without pulling in TsuiseKit.
...
extension TrackedParcel {
    /// Convenience initializer taking a typed `Carrier`. The stored designated
    /// initializer takes `carrierRaw` so it stays TsuiseKit-free.
    convenience init(
